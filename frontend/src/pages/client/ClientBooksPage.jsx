...
import { fetchClientBooks } from '../../store/clientBooksSlice';
import { useDebouncedValue } from '../../hooks/useDebouncedValue';
import SkeletonCard from '../../components/ui/SkeletonCard';
import ClientBookCard from '../../features/client/ClientBookCard';
import ClientFilterSidebar from '../../features/client/ClientFilterSidebar';
...
export default function ClientBooksPage() {
    ...
    const { items, totalPages, totalElements, currentPage, pageSize, isFirst, isLast, loading, error } = useSelector(
        (state) => state.clientBooks
    );
    ...
    const buildRequest = useCallback(
        ...
        [page, sortBy, sortDir, debouncedSearch, minPrice, maxPrice, inStockOnly, selectedGrades]
    );

    useEffect(() => {
        dispatch(fetchClientBooks(buildRequest()));
        ...
        <div>
            <div className="mb-8 sm:mb-10 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    ...
                    <p className="mt-1 text-sm sm:text-base text-slate-500 dark:text-slate-400 max-w-2xl">
                        Discover titles from verified vendors. Refine by grade, price, and availability.
                    </p>
                </div>
                {showResultCount && totalElements > 0 && (
                    <p className="text-sm text-slate-500 dark:text-slate-400 shrink-0 tabular-nums">
            ...
            <div className="lg:grid lg:grid-cols-[minmax(288px,320px)_minmax(0,1fr)] lg:gap-8 xl:gap-10">
                ...
                <div className="min-w-0 lg:pt-0.5">
                    {error && (
                        <div className="mb-6 rounded-xl border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-950/30 px-4 py-3 text-sm text-red-700 dark:text-red-300">
                            {error}
                        </div>
                    )}
                    ...
                    {loading === 'pending' && items.length === 0 ? (
                        ...
                        <div className="rounded-2xl border border-dashed border-slate-200 dark:border-slate-700 bg-white/80 dark:bg-slate-900/50 px-6 py-16 text-center">
                            ...
                            <p className="text-sm text-slate-500 dark:text-slate-500 mt-2">Try adjusting search or grades.</p>
                            {hasActiveFilters && (
                                <button
                                    type="button"
                                    onClick={handleClearAllFilters}
