import { CLIENT_GRADE_OPTIONS } from './clientGradeConstants';
...
export default function ClientFilterSidebar({
    ...
    return (
        <div className="space-y-7">
            ...
            <div>
                <label className={label} htmlFor="client-filter-search">Search</label>
                <input
            ...
            <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/30 px-3 py-3">
                <label className="flex items-center justify-between gap-3 cursor-pointer">
                    <span className="text-sm font-medium text-slate-800 dark:text-slate-200">In stock only</span>
                    <button
                        type="button"
                        role="switch"
                        ...
                        className={`relative inline-flex h-7 w-12 shrink-0 rounded-full border-2 border-transparent transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900 ${
                            inStockOnly ? 'bg-indigo-600' : 'bg-slate-200 dark:bg-slate-600'
                        }`}
                    >
                        <span
